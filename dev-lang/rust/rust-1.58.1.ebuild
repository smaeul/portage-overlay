...
SRC_URI="
	...
	!system-bootstrap? (
		...
		ppc64? (
			big-endian?  ( $(rust_arch_uri powerpc64-gentoo-linux-musl   rust-${RUST_STAGE0_VERSION} ) )
			!big-endian? ( $(rust_arch_uri powerpc64le-gentoo-linux-musl rust-${RUST_STAGE0_VERSION} ) )
		)
	)
"
...
IUSE="big-endian clippy cpu_flags_x86_sse2 debug dist doc miri nightly parallel-compiler rls rustfmt rust-src system-bootstrap system-llvm test wasm ${ALL_LLVM_TARGETS[*]}"
...
PATCHES=(
	...
	"${FILESDIR}"/${PV}/0030-libc-0.2.106-linkage.patch
...
src_configure() {
	local rust_target="" rust_targets="\"${CHOST}\"" arch_cflags

	# Collect rust target names to compile standard libs for all ABIs.
	for v in $(multilib_get_enabled_abi_pairs); do
		rust_targets="${rust_targets},\"$(rust_abi $(get_abi_CHOST ${v##*.}) | sed s/gnu/musl/)\""
	done
	if use wasm; then
